  ...
  return (
    <div className="space-y-4 text-zinc-900 bg-zinc-50/50 p-4 rounded-3xl border border-zinc-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <h4 className="font-bold text-[10px] uppercase tracking-wider text-zinc-500">{dictionary.fleet?.category || "Vehicle Category"}</h4>
          </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <h4 className="font-bold text-[10px] uppercase tracking-wider text-zinc-500">{dictionary.fleet?.transmission || "Gear Shift"}</h4>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {transmissionsList.map((transmission: string) => {
              ...
              return (
                ...
                >
                  {transmission === 'AUTOMATIC' ? dictionary.common?.automatic || "AUTOMATIC" :
                    transmission === 'MANUAL' ? dictionary.common?.manual || "MANUAL" : transmission}
                </Button>
              )
            })}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <h4 className="font-bold text-[10px] uppercase tracking-wider text-zinc-500">{dictionary.fleet?.fuel || "Fuel Type"}</h4>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {fuelTypesList.map((fuel: string) => {
              ...
              return (
                ...
                >
                  {dictionary.common?.[fuel.toLowerCase()] || fuel}
                </Button>
              )
            })}
          </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <h4 className="font-bold text-[10px] uppercase tracking-wider text-zinc-500">{dictionary.fleet?.seats_and_options || "Seats & Options"}</h4>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {seatsList.map((seat: number) => {
              ...
              return (
                ...
                >
                  {seat} {dictionary.fleet?.seats || "Seats"}
                </Button>
              )
            })}
            ...
            >
              {dictionary.fleet?.guaranteed_model || "Guaranteed Model"}
            </Button>
          </div>
        </div>
      </div>
