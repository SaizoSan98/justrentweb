...
export function BookingModal({ isOpen, onClose, car, searchParams, extras }: BookingModalProps) {
   ...
   // Calculate price based on duration using the base price (set by server to Renteon dailyRate)
                                 ...
                                 >
                                    <div className="flex items-center gap-3">
                                       ...
                                       <div>
                                          ...
                                          <div className="text-xs text-zinc-500">Includes all mileage logic</div>
                                       </div>
                                    </div>
                                    <span className="font-bold text-sm text-zinc-600">
                                       {car.unlimitedMileagePrice ? `+${car.unlimitedMileagePrice} €/day` : '0 €'}
                                    </span>
                                 </div>
                              </div>
                           </div>
                           ...
                           {sortedInsurance.map((ins) => (
                              ...
                              >
                                 ...
                                 <div className="text-right">
                                    <div className="font-bold text-xl">{ins.pricePerDay === 0 ? "Included" : `+${ins.pricePerDay.toLocaleString()} €/day`}</div>
                                    {ins.pricePerDay > 0 && <div className="text-xs text-zinc-500">Total: {(ins.pricePerDay * days).toLocaleString()} €</div>}
                                 </div>
                              </div>
                           ))}
                        </div>
