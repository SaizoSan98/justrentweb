...
export async function FeatureGrid({ dictionary }: { dictionary?: any }) {
  const initialCount = await getAvailableCarCount()
  const t = (key: string) => dictionary?.featureGrid?.[key] || key;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-auto lg:h-[600px]">
          ...
          <div className="lg:col-span-5 flex flex-col gap-6 h-full">
            ...
            <div className="relative flex-1 rounded-3xl overflow-hidden bg-zinc-900 text-white p-8 md:p-10 flex flex-col justify-between group min-h-[300px]">
              ...
              <div className="relative z-10">
                <h3 className="text-3xl font-bold mb-2 leading-tight">{t('explore_title')}</h3>
                <p className="text-zinc-300 mb-6 text-sm">{t('explore_subtitle')}</p>
                <Button asChild variant="skewed" className="px-6">
                  <Link href="/fleet">{t('booking_now')} <ArrowRight className="w-4 h-4 ml-2" /></Link>
                </Button>
              </div>
            </div>
            ...
            <div className="flex-1 min-h-[250px]">
              <VehicleCounter initialCount={initialCount} title={t('vehicles_available')} />
            </div>
