...
export function Hero({ dictionary, carCount }: { dictionary?: Dictionary; carCount?: number }) {
  const t = (key: string) => {
    return (dictionary as any)?.hero?.[key] || key;
  };
        ...
        >
          ...
          <div className="bg-black/60 backdrop-blur-xl border border-white/20 p-4 rounded-2xl transform translate-y-4 opacity-0 group-hover:opacity-100 transition-all duration-300 shadow-2xl">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-white font-bold text-base">{carCount || 50}+ {t('stats_cars')}</div>
                <div className="text-white/60 text-[10px] uppercase tracking-wider font-medium">{t('available_now')}</div>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="bg-black/60 backdrop-blur-xl border border-white/20 p-4 rounded-2xl transform translate-y-4 opacity-0 group-hover:opacity-100 transition-all duration-300 shadow-2xl">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-white font-bold text-base">20k+ {dictionary?.common?.km || 'km'}</div>
                <div className="text-white/60 text-[10px] uppercase tracking-wider font-medium">{t('total_distance')}</div>
              </div>
            </div>
          </div>
        </motion.div>
