...
async function liveSyncDebug() {
    ...
    for (const cat of targetCats) {
        console.log(`\n--- Processing ${cat.SIPP} (ID: ${cat.Id}) ---`);

        // Simulating the price check
        ...
        if (!res.ok) { console.log("   ❌ Price check failed"); continue; }
        const calcData = await res.json();
        const price = Math.round(calcData.Total / 3);
        console.log(`   💰 Price: ${price} EUR/day`);
        ...
        if (cat.CarModels) {
            for (const model of cat.CarModels) {
                ...
                const lp = `RT-${cat.SIPP}-${renteonId}`;

                console.log(`   🚗 Attempting to sync: ${make} ${modelName} (ID: ${renteonId}, LP: ${lp})`);
                ...
                try {
                    ...
                    const result = await prisma.car.upsert({
                        where: { renteonId: renteonId } as any,
                        update: carData as any,
                        create: carData as any
                    });
                    console.log(`   ✅ DB Result: ${result.id} (${result.make} ${result.model})`);
                } catch (err: any) {
...
liveSyncDebug().finally(() => prisma.$disconnect());
